...
def _FN_NOT_PORTED_YET_(func_name):
    def raise_err(self, *args, **kwargs):
        err_msg = "qt5_backport has not ported '%s' yet" % func_name
        raise NotImplementedError(err_msg)
    return raise_err

def _FN_OBSOLETE_(func_name):
    def raise_err(self, *args, **kwargs):
        err_msg = ("Function is obsolete with no replacement in "
                   "Qt5: '%s'" % func_name)
        raise NotImplementedError(err_msg)
    return raise_err

def reassemble_QtGui():
...
def _patch_QPainter():
    ...
    def _drawPixmapFragments(self, *args, **kwargs):
        ...
        if qt_backport.CURRENT_EMULATOR != qt_backport.PYQT4:
            ...
            # 
            if type(args[0][0]) != _PyQt5_QtCore.QRectF:
